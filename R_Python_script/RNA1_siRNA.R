...
setwd("F:/Project/project_Morus_virus/depth_total/siRNA_depth")
prefix <- "TRINITY_DN7047_c0_g2_i1"
depth_data <- read.table(paste0(prefix, "_siRNA.depth.list"), header = T)
RAW_data <- depth_data[, c("bp", "depth")]
# 绘制图形时使用合并后的数据
depth_plot <- ggplot(data = RAW_data, aes(x = bp, y = depth)) +
  geom_area(linetype = "solid", linewidth = 1, alpha = 1, fill = "#009E73") +
  ggtitle("RNA1_siRNA") +
  ...
  scale_x_continuous(expand = expansion(mult = c(0, 0.03)), breaks = seq(from = 0, to = 300000, by = 500))
ggsave("RNA1_siRNA.svg", plot = depth_plot, width = 8, height = 6, units = "in")
# 显示图形
print(depth_plot)
